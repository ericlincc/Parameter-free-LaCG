...
def numba_project_onto_active_set(
    ...
    active_set_matrix = np.zeros(
        (len(active_set), len(active_set[0])), active_set[0].dtype
    )
    ...
    (
        ...
        gap_values1,
    ) = accelerated_projected_gradient_descent_over_simplex(
        quadratic,
        linear,
        active_set_matrix,
        barycentric_coord,
...
def accelerated_projected_gradient_descent_over_simplex(
    ...
    it_count = 0
    gap_values = [FWGap]
    while (FW_criterion and FWGap > tolerance) or (
        not FW_criterion
        and coefficient * np.linalg.norm(x - reference_point) ** 2 < FWGap
    ):
        # print(FWGap)
        x_old = x.copy()
