...
class FrankWolfe_sparse(_AbstractAlgorithm):
    """
    ...
    def run(
        ...
        if point_initial is None:
            ...
            point_x = Point_sparse(np.asarray(vertex.todense()).squeeze(), (1.0,), (vertex,))
        else:
        ...
        strong_wolfe_gap = (point_x.support[index_max] - v).dot(grad).squeeze()
        dual_gap = (point_x.support[0] - v).dot(grad).squeeze()
        ...
        while True:
            ...
            if self.fw_variant == "AFW":
                point_x, dual_gap_prev, strong_wolfe_gap_prev = away_step_fw_sparse(
                    objective_function,
                    feasible_region,
                    point_x,
                    self.step_size_param,
            ...
            if self.fw_variant == "PFW":
                point_x, dual_gap_prev, strong_wolfe_gap_prev = pairwise_step_fw_sparse(
                    objective_function,
                    feasible_region,
                    point_x,
                    self.step_size_param,
            ...
            if self.fw_variant == "FW":
                point_x, dual_gap_prev, strong_wolfe_gap_prev = step_fw_sparse(
                    objective_function,
                    feasible_region,
                    point_x,
                    self.step_size_param,
            ...
            if self.fw_variant == "lazy":
                point_x, dual_gap_prev, strong_wolfe_gap_prev = fw_away_lazy_sparse(
                    objective_function,
                    feasible_region,
                    point_x,
                    self.step_size_param,
...
def step_fw_sparse(objective_function, feasible_region, point_x, step_size_param):
    grad = objective_function.evaluate_grad(point_x.cartesian_coordinates)
    v = feasible_region.lp_oracle(grad)
    ...
    if alpha != alpha_max:
        ...
        return Point_sparse(v, (1.0,), (v,)), wolfe_gap, 0.0


def away_step_fw_sparse(objective_function, feasible_region, point_x, step_size_param):
    grad = objective_function.evaluate_grad(point_x.cartesian_coordinates)
    v = feasible_region.lp_oracle(grad)
    point_a, index_max = feasible_region.away_oracle(grad, point_x)
    wolfe_gap = point_x.cartesian_coordinates.dot(grad) - v.dot(grad).squeeze()
    strong_wolfe_gap = (point_x.support[index_max] - v).dot(grad).squeeze()
    if 2.0*wolfe_gap > strong_wolfe_gap:
        ...
        alpha = step_size(
            objective_function,
            point_x.cartesian_coordinates,
            v.A.squeeze() - point_x.cartesian_coordinates,
            grad,
            alpha_max,
            step_size_param,
        )
        if alpha != alpha_max:
            ...
            return (
                Point_sparse(v.toarray().squeeze(), (1.0,), (v,)),
                wolfe_gap,
                strong_wolfe_gap,
            )
    else:
...
def pairwise_step_fw_sparse(objective_function, feasible_region, point_x, step_size_param):
    grad = objective_function.evaluate_grad(point_x.cartesian_coordinates)
    v = feasible_region.lp_oracle(grad)
    wolfe_gap = grad.dot(point_x.cartesian_coordinates - v)
    point_a, index_max = feasible_region.away_oracle(grad, point_x)
...
def dipfw_sparse(objective_function, feasible_region, point_x, step_size_param):
    grad = objective_function.evaluate_grad(point_x.cartesian_coordinates)
    v = feasible_region.lp_oracle(grad)
    grad_aux = grad.copy()
...
def away_step_fw(objective_function, feasible_region, point_x, step_size_param):
    ...
    v = feasible_region.lp_oracle(grad)
    point_a, index_max = feasible_region.away_oracle(grad, point_x)
    wolfe_gap = (point_x.cartesian_coordinates - v).dot(grad)
    strong_wolfe_gap = (point_x.support[index_max] - v).dot(grad)
    if 2.0*wolfe_gap > strong_wolfe_gap:
        alpha_max = 1.0
        alpha = step_size(
            objective_function,
