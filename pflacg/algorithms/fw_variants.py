    ...
    def run(
        ...
        while not exit_criterion.has_met_exit_criterion(run_status):
            ...
            if self.fw_variant == "DIPFW":
                x, dual_gap, strong_wolfe_gap = DIPFW(
...
class FrankWolfe_backup(_AbstractAlgorithm):
    ...
    def run(
        ...
        if initial_point is None or active_set is None or lambdas is None:
        ...
        while not exit_criterion.has_met_exit_criterion(run_status):
            ...
            if self.fw_variant == "DIPFW":
                x, dual_gap, strong_wolfe_gap = DIPFW_backup(
                    objective_function, feasible_region, x, self.step_size_param
                )
...
def step_fw_backup(
    objective_function, feasible_region, x, active_set, lambdas, step_size_param
):
    grad = objective_function.evaluate_grad(x)
    v = feasible_region.lp_oracle(grad)
    d = v - x
    alphaMax = 1.0
    alpha = step_size(objective_function, x, d, grad, alphaMax, step_size_param)
    if alpha != alphaMax:
...
def FW_away_lazy_quick_exit_backup(
    objective_function,
    feasible_region,
    x,
    active_set,
