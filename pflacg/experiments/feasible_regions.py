...
class ConstrainedL1BallPolytope(_AbstractFeasibleRegion):
    def __init__(self, l1_regularization, dim, const_matrix_ineq = None, const_vector_ineq = None, const_matrix_eq = None, const_vector_eq = None, solver_type = "cvxopt", scipy_solver = "revised simplex", sparse_solver = False):
        self.dim = dim
        ...
        if(sparse_solver):
            assert solver_type == "cvxopt", "scipy solver cannot handle sparse matrices."
        ...
        if(const_matrix_ineq is not None and const_vector_ineq is not None):
            ...
            assert dim_ineq_constraints == self.dim, "Dimension of the inequality constraints does not match the dimensionality of the problem."
            self.G = np.vstack((np.hstack((const_matrix_ineq, -const_matrix_ineq)), -np.identity(simplex_dimensionality)))
            self.h = np.append(const_vector_ineq, np.zeros(simplex_dimensionality))
            if(solver_type == "cvxopt"):
                self.G = matrix(self.G, (simplex_dimensionality + num_ineq_constraints, simplex_dimensionality))
                if(sparse_solver):
                    self.G = sparse(self.G)
                self.h = matrix(self.h, (simplex_dimensionality + num_ineq_constraints, 1))
        else:
        ...
        if(const_matrix_eq is not None and const_vector_eq is not None):
            ...
            assert dim_eq_constraints == self.dim, "Dimension of the equality constraints does not match the dimensionality of the problem."
            self.A = np.vstack((np.hstack((const_matrix_eq, -const_matrix_eq)), np.ones(simplex_dimensionality)))
            self.b = np.append(const_vector_eq, self.l1_regularization).tolist()
            if(solver_type == "cvxopt"):
                self.A = matrix(self.A, (1 + num_eq_constraints, simplex_dimensionality))
            ...
            if(solver_type == "cvxopt"):
                ...
                self.A = np.ones(simplex_dimensionality).reshape((simplex_dimensionality, 1))
    ...
    def lp_oracle(self, x):
        ...
        if(self.solver_type == 'cvxopt'):
            sol = solvers.lp(matrix(cost_vector), self.G, self.h, self.A, self.b)
            assert sol['status'] == "optimal", "Algorithm did not converge."
            optimum = np.array(sol['x'])
            return (optimum[:int(len(optimum)/2)] - optimum[int(len(optimum)/2):]).flatten()
        else:
            res = linprog(cost_vector, A_ub=self.G, b_ub=self.h, A_eq=self.A.T, b_eq = self.b, method = self.scipy_solver, bounds = (-np.inf, np.inf))
...
class GeneralPolytope(_AbstractFeasibleRegion):
    def __init__(self, dim, const_matrix_ineq = None, const_vector_ineq = None, const_matrix_eq = None, const_vector_eq = None, solver_type = "cvxopt", scipy_solver = "revised simplex", sparse_solver = False):
        self.dim = dim
        self.solver_type = solver_type
        assert solver_type == "cvxopt" or solver_type == "scipy", "Wrong solver type"
        if(solver_type == "cvxopt"):
        ...
        if(self.solver_type == 'cvxopt'):
            if(self.G is not None and self.h is not None):
                if(self.A is not None and self.b is not None):
                    sol = solvers.lp(matrix(x), G = self.G, h = self.h, A = self.A, b = self.b)
                else:
                    sol = solvers.lp(matrix(x), G = self.G, h = self.h)
            else:
                if(self.A is not None and self.b is not None):
                    sol = solvers.lp(matrix(x), A = self.A, b = self.b)
                else:
            ...
            if(self.G is not None and self.h is not None):
                if(self.A is not None and self.b is not None):
                    res = linprog(x, A_ub=self.G, b_ub=self.h, A_eq=self.A.T, b_eq = self.b, method = self.scipy_solver, bounds = (-np.inf, np.inf))
                else:
                    res = linprog(x, A_ub=self.G, b_ub=self.h, method = self.scipy_solver, bounds = (-np.inf, np.inf))
            else:
                if(self.A is not None and self.b is not None):
                    res = linprog(x, A_eq=self.A.T, b_eq = self.b, method = self.scipy_solver, bounds = (-np.inf, np.inf))
                else:
