...
from pflacg.experiments.experiments_helper import max_vertex, max_vertex_sparse
...
class BirkhoffPolytope_sparse(_AbstractFeasibleRegion):
    def __init__(self, dim):
        self.dim = dim
        self.mat_dim = int(np.sqrt(dim))
    ...
    @property
    def initial_point(self):
        return csr_matrix(np.identity(self.mat_dim).flatten())
    ...
    def lp_oracle(self, d):
        ...
        solution = np.zeros((self.mat_dim, self.mat_dim))
        solution[matching] = 1
        return csr_matrix(solution.reshape(self.dim))

    def away_oracle(self, grad, point_x):
        return max_vertex_sparse(grad, point_x.support)

class ProbabilitySimplexPolytope_sparse(_AbstractFeasibleRegion):
    def __init__(self, dim):
        self.dim = dim
    ...
    @property
    def initial_active_set(self):
        return [csr_matrix(self.initial_point())]
