...
def away_step_fw(objective_function, feasible_region, point_x, step_size_param):
    ...
    if FWGap > grad.dot(point_a.cartesian_coordinates - point_x.cartesian_coordinates):
        ...
        if alpha != alphaMax:
            ...
            return point_x + alpha*(point_v - point_x), FWGap, grad.dot(point_a.cartesian_coordinates - point_v.cartesian_coordinates)
        else:
            return Point(v, (v,), (1.0,)), FWGap, grad.dot(point_a.cartesian_coordinates - point_v.cartesian_coordinates)
    else:
        ...
        return point_x, FWGap, grad.dot(point_a.cartesian_coordinates - v)

def pairwise_step_fw( objective_function, feasible_region, point_x, step_size_param):
    grad = objective_function.evaluate_grad(point_x)
    v = feasible_region.lp_oracle(grad)
    point_a, indexMax = feasible_region.away_oracle(grad, point_x)
    # Find the weight of the extreme point a in the decomposition.
    alphaMax = point_a.barycentric_coordinates[indexMax]
    alpha = step_size(
        objective_function, point_x.cartesian_coordinates, v - point_a.cartesian_coordinates, grad, alphaMax, step_size_param
    )
    flag, point_v = point_x.is_vertex_in_support(v)
    if flag == False:
        point_x = Point(point_x.cartesian, tuple(list(point_x.barycentric_coordinates).append(0.0)), point_v.support)
        point_a = Point(point_a.cartesian, tuple(list(point_a.barycentric_coordinates).append(0.0)), point_a.support)
    point_x = point_x + alpha * (point_v - point_a)
    if alpha == alphaMax:
        point_x = point_x.delete_vertex_in_support(indexMax)
    return point_x, grad.dot(point_x.cartesian_coordinates - point_v.cartesian_coordinates), grad.dot(point_a.cartesian_coordinates - point_v.cartesian_coordinates)
...
def step_fw_backup(objective_function, feasible_region, x, step_size_param):
    grad = objective_function.evaluate_grad(x)
    v = feasible_region.lp_oracle(grad)
    d = v - x
    alphaMax = 1.0
